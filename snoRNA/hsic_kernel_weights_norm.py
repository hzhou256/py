...
def kernel_RBF(X, Y):
    ...
    k = np.tile(np.sum(X ** 2, 1), (np.size(Y, 0), 1)).T + np.tile(np.sum(Y ** 2, 1), (np.size(X, 0), 1)) - 2 * np.dot(
        X, Y.T)
    return k
...
def obj_function(w, Mi, ai, regcoef1, regcoef2):
    J = -1 * np.dot(w.T, ai) + regcoef1 * np.dot(np.dot(w.T, Mi), w) + regcoef2 * (
        np.linalg.norm(w, ord=2, keepdims=True)) ** 2
    return J
