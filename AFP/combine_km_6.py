...
for ds in range(1):
    ...
    for mkl in range(0,3):
        ...
        f5 = np.loadtxt('D:/Study/Bioinformatics/AFP/kernel_matrix/' + name_ds +'/KM_train_gaussian/KM_gaussian_CTD_train.csv', delimiter = ',')
        f6 = np.loadtxt('D:/Study/Bioinformatics/AFP/kernel_matrix/' + name_ds +'/KM_train_gaussian/KM_gaussian_DPC_train.csv', delimiter = ',')

        weight_v = np.loadtxt('D:/Study/Bioinformatics/AFP/kernel_matrix_6/' + name_ds +'/KM_train_gaussian/' + name_mkl + '_weight.txt', delimiter = ',', usecols = 0)
        print(weight_v)
        K = f1 * weight_v[0] + f2 * weight_v[1] + f3 * weight_v[2] + f4 * weight_v[3] + f5 * weight_v[4] + f6 * weight_v[5]
        print(K)

        with open('D:/Study/Bioinformatics/AFP/kernel_matrix_6/' + name_ds +'/KM_train_gaussian/combine_gaussian_' + name_mkl + '_train.csv', 'w', newline='') as csvfile:
        ...
        f5 = np.loadtxt('D:/Study/Bioinformatics/AFP/kernel_matrix/' + name_ds +'/KM_test_gaussian/KM_gaussian_CTD_test.csv', delimiter = ',')
        f6 = np.loadtxt('D:/Study/Bioinformatics/AFP/kernel_matrix/' + name_ds +'/KM_test_gaussian/KM_gaussian_DPC_test.csv', delimiter = ',')

        temp = np.array([f1, f2, f3, f4, f5, f6])
        Kernels_list = temp

        K = f1 * weight_v[0] + f2 * weight_v[1] + f3 * weight_v[2] + f4 * weight_v[3] + f5 * weight_v[4] + f6 * weight_v[5]
        print(K)

        with open('D:/Study/Bioinformatics/AFP/kernel_matrix_6/' + name_ds +'/KM_test_gaussian/combine_gaussian_' + name_mkl + '_test.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            for row in K:
                writer.writerow(row)
            csvfile.close()
