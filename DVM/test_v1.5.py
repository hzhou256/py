...
for i in range(5, 6):
    ...
    grid = GridSearchCV(Class_KDVM_v1_5.KDVM(kernel = 'rbf'), parameters, n_jobs = -1, cv = cv, verbose = 1)
    grid.fit(X, y)
    gamma = grid.best_params_['gamma']
    n_neighbors = grid.best_params_['n_neighbors']

    clf = Class_KDVM_v1_5.KDVM(kernel = 'rbf', gamma = gamma, n_neighbors = n_neighbors)
