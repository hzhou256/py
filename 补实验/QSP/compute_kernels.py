...
for it in range(0, 5):
    ...
    print(name)
    X_train = np.loadtxt('D:/Study/Bioinformatics/补实验/QSP/features/train_'+name+'.csv', delimiter = ',')
    #X_test = np.loadtxt('D:/Study/Bioinformatics/补实验/QSP/features/test_'+name+'.csv', delimiter = ',')
    ...
    with open('D:/Study/Bioinformatics/补实验/QSP/kernels/K_train_'+name+'.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        for row in K_train:
            writer.writerow(row)
        csvfile.close()
    #with open('D:/Study/Bioinformatics/补实验/QSP/kernels/K_test_'+name+'.csv', 'w', newline='') as csvfile:
    #    writer = csv.writer(csvfile)
    #    for row in K_test:
    #        writer.writerow(row)
    #    csvfile.close()
