...
# Divide data set X into two subsets according to their labels y
def split(X, y): 
    cnt = Counter(y)
    n_pos = cnt[1] 
    n_neg = cnt[-1] 
...
def class_center_membership(X, y, delta): # X: data, y: label, delta: parameter
...
def get_radius_square(x_i, y_i, X, Y, K): # calculate distance between two points in feature space
    cnt = Counter(Y)
    n_pos = cnt[1]
    n_neg = cnt[-1]
    X_pos, X_neg = split(X, Y)
...
def FSVM_2_membership(X, y, delta, K): # X: data, y: label, delta: parameter, K: kernel function
    X_pos, X_neg = split(X, y)
...
def get_kth_evec(X, K, k, N): # calculate k_th largest eigenvector
...
def get_beta(x_i, X, K, N, j): # calculate beta_j
...
def get_gamma(X, K, N, l): # calculate gamma_l
...
def reconstruction_error(x, X, y, K, k): # calculate reconstruction_error for x
    N = len(y)
