...
for ds in range(2, 3):
    ...
    for it in range(1):
        ...
        plt.plot(fpr, tpr, label = 'Naive Bayes - AUC: 0.9121')
        ...
        y_score = get_y_score(y_score)
        fpr, tpr, thresholds = metrics.roc_curve(y_test, y_score)
        plt.plot(fpr, tpr, label = 'Random Forest - AUC: 0.9739')
        ...
        y_score = get_y_score(y_score)
        fpr, tpr, thresholds = metrics.roc_curve(y_test, y_score)
        plt.plot(fpr, tpr, label = 'Decision Tree - AUC: 0.9071')
        ...
        y_score = get_y_score(y_score)
        fpr, tpr, thresholds = metrics.roc_curve(y_test, y_score)
        plt.plot(fpr, tpr, label = 'SVM - AUC: 0.9761')        
        ...
        y_score = get_y_score(y_score)
        fpr, tpr, thresholds = metrics.roc_curve(y_test, y_score)
        plt.plot(fpr, tpr, label = 'Our Model - AUC: 0.9779')  

        plt.legend()
        plt.xlabel('False positive rate')
        plt.ylabel('True positive rate')
        plt.savefig("D:/论文/图表/ROC_PR/ROC_" + name_ds + ".png")
        plt.show()