...
def get_frag(fasta_dict,positive_dict,idlist,nb_windows, empty_aa,focus):
    ...
    for id in idlist: #for sort
        ...
        for pos in range(len(seq)):
            ...
            if len(left_seq) < nb_windows-1:
                if empty_aa is None:
                     continue
                nb_lack = nb_windows - 1 - len(left_seq)
...
def extract_fragment_for_train(fasta_file,windows=15,empty_aa='-',focus=('')):
    fasta_dict,positive_dict,idlist = read_fasta(fasta_file) 
    
    frag,ids,poses,focus = get_frag(fasta_dict,positive_dict,idlist, windows, empty_aa,focus)
    return frag,ids,poses,focus

def extract_fragment_for_predict(fasta_file,windows=15,empty_aa='-',focus=('')):
    fasta_dict,positive_dict,idlist = read_fasta(fasta_file) 
