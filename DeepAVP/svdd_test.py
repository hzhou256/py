...
for it in range(0, 3):
    ...
    grid = GridSearchCV(Fuzzy_SVM.FSVM_Classifier(kernel = 'rbf', membership = 'SVDD', proj = 'beta'), parameters, n_jobs = -1, cv = cv, verbose = 1)
    grid.fit(X_train, y_train)
    C = grid.best_params_['C']
    gamma = grid.best_params_['gamma']
    nu = grid.best_params_['nu']

    clf = Fuzzy_SVM.FSVM_Classifier(C = C, gamma = gamma, nu = nu, kernel = 'rbf', membership = 'SVDD', proj = 'beta')
